...
import type { ModelRequest, Profile, ModelRequestStatus } from "@/types/database";

export type RequestWithCreator = ModelRequest & {
...
export function useRequests(options: UseRequestsOptions = {}) {
  ...
  return useQuery({
    queryKey: ["model_requests", status],
    queryFn: async (): Promise<RequestWithCreator[]> => {
      let query = supabase
        .from("model_requests")
        .select(
          `
      ...
      let requests = (data || []) as RequestWithCreator[];
...
export function useRequest(id: string) {
  return useQuery({
    queryKey: ["model_request", id],
    queryFn: async (): Promise<RequestWithCreator | null> => {
      ...
      if (error) throw error;
      return data as RequestWithCreator;
    },
    enabled: !!id,
  });
}
